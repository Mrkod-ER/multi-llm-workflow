...
from app.schemas.api import WorkflowRunRequest, WorkflowRunResponse, NodeExecutionResult
from app.engine.graph import validate_dag, get_node_dependencies
...
class WorkflowRunner:
    ...
    async def run(self, workflow_id: str = "run_1") -> WorkflowRunResponse:
        ...
        try:
            sorted_node_ids = validate_dag(nodes=self.workflow.nodes, edges=self.workflow.edges)
        ...
        status = "success" if all(r.error is None for r in results.values()) and len(results) == len(sorted_node_ids) else "error"
    ...
    async def run_stream(self, workflow_id: str = "run_1") -> AsyncGenerator[Dict, None]:
        queue = asyncio.Queue()
        ...
        async def _execute_graph():
            ...
            node_map = {node.id: node for node in self.workflow.nodes}
            context = ExecutionContext(memory=self.memory, workflow_id=workflow_id, stream_queue=queue)
