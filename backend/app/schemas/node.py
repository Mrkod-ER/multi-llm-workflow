...
class LLMNodeData(BaseModel):
    ...
    system_prompt: str = Field(default="", description="The system prompt telling the LLM how to behave.")
    model: str = Field(..., description="The name of the model to use (e.g., 'gpt-4o', 'llama3').")
    provider: str = Field(..., description="The provider hosting the model (e.g., 'openai', 'ollama').")

class OutputNodeData(BaseModel):
    """Data payload for an Output node."""
    type: Literal[NodeType.OUTPUT] = NodeType.OUTPUT
    result: str = Field(default="", description="The final result populated after execution.")

# Discriminated union for node data allowing fast, type-safe parsing based on 'type' field
NodeData = Annotated[
    Union[InputNodeData, LLMNodeData, OutputNodeData], 
