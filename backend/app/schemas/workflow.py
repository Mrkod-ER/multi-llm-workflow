...
class WorkflowMetadata(BaseModel):
    ...
    description: Optional[str] = Field(
        None, description="A brief description of what the workflow does."
    )
...
class Workflow(BaseModel):
    ...
    nodes: List[Node] = Field(
        default_factory=list, description="A list of nodes making up the DAG."
    )
    edges: List[Edge] = Field(
        default_factory=list,
        description="A list of directed edges connecting the nodes.",
    )
